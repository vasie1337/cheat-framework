...
std::vector<uintptr_t> scan_entity_pointers(Core& core) {
    ...
    for (size_t i = 0; i < valid_list_ptrs.size(); i++) {
        core.m_access_adapter->add_scatter(valid_list_ptrs[i],
                                           entity_chunks[i].data(),
                                           entity_chunks[i].size());
    }
    core.m_access_adapter->execute_scatter();
    ...
    for (int i = 0; i < entity_chunks.size(); i++) {
        for (int j = 0; j < LIST_ENTRIES; j++) {
            ...
            uintptr_t* identity =
                reinterpret_cast<uintptr_t*>(entity_chunks[i].data() + (j * CENTITY_IDENTITY_SIZE));
            if (identity[0] != 0) {
...
void update_player_cache(Core& core) {
    ...
    if (!uncached_players.empty()) {
        for (auto* player : uncached_players) {
            core.m_access_adapter->add_scatter(player->entity.instance +
                                                   CCSPlayerController::m_hPlayerPawn,
                                               &player->controller_pawn,
                                               sizeof(player->controller_pawn));
        }
        core.m_access_adapter->execute_scatter();

        for (auto* player : uncached_players) {
            core.m_access_adapter->add_scatter(
                game_cache.ent_list + 0x8 * ((player->controller_pawn & 0x7FFF) >> 0X9) + 0x10,
                &player->list_pawn,
                sizeof(player->list_pawn));
        }
        core.m_access_adapter->execute_scatter();

        for (auto* player : uncached_players) {
            core.m_access_adapter->add_scatter(player->list_pawn +
                                                   0x78 * (player->controller_pawn & 0x1FF),
                                               &player->player_pawn,
                                               sizeof(player->player_pawn));
        }
        core.m_access_adapter->execute_scatter();

        for (auto* player : uncached_players) {
            core.m_access_adapter->add_scatter(player->player_pawn + C_BaseEntity::m_pGameSceneNode,
                                               &player->entity.game_scene_node,
                                               sizeof(player->entity.game_scene_node));
        }
        core.m_access_adapter->execute_scatter();

        for (auto* player : uncached_players) {
            core.m_access_adapter->add_scatter(player->entity.game_scene_node +
                                                   CSkeletonInstance::m_modelState + 0x80,
                                               &player->bone_array,
...
static void reader(Core& core) {
    ...
    if (core.m_update_manager->should_update("globals", GameCache::GLOBALS_INTERVAL)) {
        core.m_access_adapter->add_scatter(game_cache.client_dll + dwLocalPlayerPawn,
                                           &game_cache.local_player_ptr,
                                           sizeof(game_cache.local_player_ptr));
        core.m_access_adapter->add_scatter(game_cache.client_dll + dwEntityList,
                                           &game_cache.ent_list,
                                           sizeof(game_cache.ent_list));
    }
...
static void renderer(Core& core) {
    ...
    for (const auto& player : game_cache.players) {
        ...
        for (const auto& bone : player.bones) {
            ...
            if (core.m_projection_utils->WorldToScreen(bone.position,
                                                       screen_position,
                                                       game_cache.view_matrix)) {
    ...
    for (const auto& entity : game_cache.entities) {
        if (!entity.data_cached || entity.designer_name == "cs_player_controller")
            continue;

        vec2_t<float> screen_position;
        if (core.m_projection_utils->WorldToScreen(entity.position,
                                                   screen_position,
                                                   game_cache.view_matrix)) {
            draw_list->AddCircleFilled(ImVec2(screen_position.x, screen_position.y),
                                       2.f,
